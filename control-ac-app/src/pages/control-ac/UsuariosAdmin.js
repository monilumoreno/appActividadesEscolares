...
const UsuariosAdmin = () => {
    ...
    const cargarUsuarios = async (idPerfil) => {   
        ...
        let response
        if (idPerfil == 0) {
            response = await APIInvoke.invokeGET(`/usuarios`);
        } else {
                        ...
                        <div className="col-3">
                            <div className="input-group">
                                ...
                                <select className="form-select"
                                    ...
                                    onChange={(e) => cargarUsuarios(e.target.value)} 
                                    required
                                >
                                    {
                                        perfiles.map(
                                            ...
                                            resultados.map(
                                                item =>                                                    
                                                    <tr key={item.idUsuario}>
                                                        ...
                                                        <td nowrap="">
                                                            <Link to={`/usuarios-editar/${item.idUsuario}`} className="btn btn-sm btn-primary w-70px me-1">Editar</Link>
                                                            <button onClick={(e) => confrimarElimnacion(e, item.idUsuario)} className="btn btn-sm btn-danger w-70px">Eliminar</button>
                                                        </td>
                                                    </tr>
                                            )
                                        }
